...
function resolveAttendanceJustification(item = {}) {
    ...
    // Definitive proof: a justification date means the event IS justified.
    const hasGiustificationDate = !!(
        String(item.datGiustificazione || '').trim() ||
        String(item.dataGiustificazione || '').trim() ||
        String(item.datGiustifica || '').trim()
    );

    // Secondary proof: justification reason/UID present
    const hasGiustificationEvidence = !!(
        String(item.giustificaBinUid || '').trim() ||
        String(item.desGiustificazione || '').trim() ||
        String(item.motivoGiustificazione || '').trim()
    );

    // --- Priority logic ---
    // IMPORTANT: Argo's `flgDaGiustificare` is a TYPE flag ("this event CAN be justified"),
    // NOT a status flag. It is always "S" for absences/delays/exits. The actual justification
    // status is determined by `datGiustificazione` (date when justified) or explicit boolean flags.
    // Therefore `datGiustificazione` MUST take the highest priority.
    let giustificata = false;

    if (hasGiustificationDate || hasGiustificationEvidence) {
        // Priority 1: Justification date/evidence exists → definitively justified
        giustificata = true;
    } else if (giustificataFlag !== null) {
        // Priority 2: Explicit giustificata boolean flag
        giustificata = giustificataFlag;
    } else if (daGiustificareFlag === false) {
        // Priority 3: Explicitly marked as NOT needing justification
        giustificata = true;
    } else if (daGiustificareFlag === true) {
        // Priority 4: Marked as needing justification (and no counter-evidence above)
        giustificata = false;
    }
    // else: default false (no information available)
