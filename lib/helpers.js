...
function verifySessionToken(req, userId) {
    const key = process.env.ARGO_ENCRYPTION_KEY || '';
    if (!key || key.length !== SESSION_TOKEN_HEX_LENGTH || !/^[0-9a-fA-F]+$/.test(key)) return true; // not configured — skip check
    const provided = (req.headers['x-session-token'] || '').trim();
    if (!provided) return false;
    if (!new RegExp(`^[0-9a-fA-F]{${SESSION_TOKEN_HEX_LENGTH}}$`).test(provided)) return false;
