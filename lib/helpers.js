...
function decryptArgoPassword(stored) {
    ...
    if (!stored.startsWith('enc:')) {
        console.warn('⚠️ Argo password is stored in plaintext format; migration to encrypted format is recommended.');
        return stored; // backward compat: plaintext value
    }
    const key = _getEncryptionKey();
    try {
 ...
 * Returns true only when ARGO_ENCRYPTION_KEY is configured and the token is valid.
 */
...
function generatePid(school, user, index) {
    ...
    const i = String(index != null ? index : 0);
