...
// Allowed origins are read once at module load from ALLOWED_ORIGINS env var.
// Format: comma-separated list, e.g. "https://example.com,http://localhost:8080"
...
function setCorsHeaders(res, req) {
    ...
    if (_ALLOWED_ORIGINS.length === 0) {
        // No restriction configured — fall back to wildcard
        allowOrigin = '*';
    } else if (requestOrigin && _ALLOWED_ORIGINS.includes(requestOrigin)) {
        // Reflect the matched origin and add Vary to prevent caching issues
        allowOrigin = requestOrigin;
        res.setHeader('Vary', 'Origin');
    } else {
        // Origin not in allow-list — reflect first allowed origin as a safe default
        allowOrigin = _ALLOWED_ORIGINS[0];
        res.setHeader('Vary', 'Origin');
    }
