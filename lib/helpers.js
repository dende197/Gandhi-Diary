...
// ============= SUBJECT CANONICALIZATION HELPERS =============

function getSubjectCanonicalName(raw) {
    if (!raw) return 'Generico';
    const s = String(raw).trim();
    if (!s) return 'Generico';
    const lower = s.toLowerCase().replace(/[*_()[\]\-]/g, ' ').replace(/\s+/g, ' ').trim();

    // 1. Disegno e Storia dell'Arte (must precede standalone Storia)
    if (lower.includes('disegn') || lower.includes('dellarte') || lower.includes('dell arte') || lower.includes('storia dell') || (lower.includes('arte') && !lower.includes('letterat'))) {
        return "Disegno e Storia Dell'arte Triennio";
    }

    // 2. Educazione Civica (must precede standalone Storia)
    if (lower.includes('civic') || lower.includes('cittadin')) {
        return 'Educazione Civica';
    }

    // 3. Storia Triennio
    if (lower.includes('stori')) {
        return 'Storia Triennio';
    }

    // 4. Italiano
    if (lower.includes('ital') || lower.includes('letter') || lower.includes('narrat') || lower.includes('antol') || lower.includes('gramm')) {
        return 'Italiano';
    }

    // 5. Filosofia
    if (lower.includes('filos')) {
        return 'Filosofia';
    }

    // 6. Inglese
    if (lower.includes('ingl') || lower.includes('stranier') || lower.includes('english')) {
        return 'Inglese';
    }

    // 7. Informatica
    if (lower.includes('inform')) {
        return 'Informatica';
    }

    // 8. Scienze Motorie e Sportive (must precede Scienze and Fisica)
    if (lower.includes('motor') || lower.includes('sport') || lower.includes('ginnas') || (lower.includes('educazione') && lower.includes('fisic'))) {
        return 'Scienze Motorie e Sportive';
    }

    // 9. Scienze Naturali
    if (lower.includes('scienz') || lower.includes('chimic') || lower.includes('biol') || lower.includes('geol') || lower.includes('natura')) {
        return 'Scienze Naturali';
    }

    // 10. Fisica
    if (lower.includes('fisic')) {
        return 'Fisica';
    }

    // 11. Matematica
    if (lower.includes('matem') || lower.includes('algeb') || lower.includes('geom') || lower.includes('trigon')) {
        return 'Matematica';
    }

    // 12. Religione
    if (lower.includes('relig') || lower.includes('irc')) {
        return 'Religione';
    }
...
function detectSubjectFromText(text) {
    ...
    if (/\b(?:disegno|storia\s+dell['\s]?arte|arte)\b/i.test(lower)) return "Disegno e Storia Dell'arte Triennio";
    if (/\b(?:ed(?:ucazione)?\.?\s*civica|cittadinanza)\b/i.test(lower)) return 'Educazione Civica';
    if (/\b(?:storia(?:\s+triennio)?)\b/i.test(lower)) return 'Storia Triennio';
    if (/\b(?:italiano|letteratura(?:\s+italiana)?|grammatica|antologia|divina\s+commedia|dante|promessi\s+sposi)\b/i.test(lower)) return 'Italiano';
    if (/\b(?:matematica|algebra|geometria|trigonometria|equazion[ei]|disequazion[ei])\b/i.test(lower)) return 'Matematica';
    if (/\b(?:inglese|english|reading|grammar|unit\s+\d+)\b/i.test(lower)) return 'Inglese';
    if (/\b(?:filosofia|filo)\b/i.test(lower)) return 'Filosofia';
    if (/\b(?:fisica)\b/i.test(lower)) return 'Fisica';
    if (/\b(?:scienze(?:\s+naturali)?|chimica|biologia|geologia)\b/i.test(lower)) return 'Scienze Naturali';
    if (/\b(?:informatica|coding|programmazione)\b/i.test(lower)) return 'Informatica';
    if (/\b(?:scienze\s+motorie|ed(?:ucazione)?\.?\s*fisica|ginnastica)\b/i.test(lower)) return 'Scienze Motorie e Sportive';
    if (/\b(?:religione|irc)\b/i.test(lower)) return 'Religione';
