...
function resolveAttendanceJustification(item = {}) {
    ...
    } else if (daGiustificareFlag === false && giustificataFlag !== false) {
        // Priority 3 Hardened: Marked as NOT needing justification 
        // AND not explicitly marked as false. Fixes false positives for authorized escapes.
        giustificata = true;
    } else if (daGiustificareFlag === true) {
        // Priority 4: Marked as justifiable (Argo type flag) → Unjustified if no date/UID
        giustificata = false;
    } else if (hasMotiveInfo && giustificataFlag === true) {
        // Priority 5 Hardened: Motive is only proof IF giustificata flag is also true
        giustificata = true;
    }
    // else: default false (conservative)
