...
function resolveAttendanceJustification(item = {}) {
    ...
    // IMPORTANT: These might be present even if NOT yet justified (as a placeholder or category)
    const hasMotiveInfo = !!(
    ...
    if (hasValidGiustificationDate || hasTechnicalProof) {
        ...
        // Priority 4: Marked as justifiable (Argo type flag)
        // If we have motive info but NO date, we trust the "daGiustificare" flag!
        // This avoids the "April 13th" bug where a motive exists but it's not done.
        giustificata = false;
    } else if (hasMotiveInfo) {
        // Priority 5: No flags, but motive exists (weak evidence)
        giustificata = true;
    }
    // else: default false
