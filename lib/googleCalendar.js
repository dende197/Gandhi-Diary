...
/**
 * Genera un ID univoco pseudonimo per un compito.
 */
function generateArgoId(materia, data) {
    const raw = `${materia.toLowerCase().trim()}-${data}`;
...
async function syncTasksToCalendar(tasks) {
    ...
    try {
        ...
        for (const task of tasksToProcess) {
            ...
            const dateStr = task.due_date || task.datCompito || task.dataConsegna || '';
            const summary = `[${materia.toUpperCase()}] Homework`;
            const argoId = generateArgoId(materia, dateStr);

            // Finestra di ricerca: ±1 giorno per sicurezza
            const dataTarget = parseDataArgo(dateStr);
            ...
            try {
                ...
                await calendar.events.insert({
                    ...
                    requestBody: {
                        summary,
                        description,
                        start: { date: dateStr.includes('T') ? dateStr.split('T')[0] : dateStr },
                        end: { date: dateStr.includes('T') ? dateStr.split('T')[0] : dateStr },
...
async function removeDuplicates() {
    ...
    try {
        ...
        for (const key in groups) {
            ...
            if (list.length > 1) {
                // Ordina per data creazione (se disponibile) o mantieni il primo
                // In Google Calendar 'created' è la data di creazione
                list.sort((a, b) => new Date(b.created) - new Date(a.created));

                // Mantieni il più recente (il primo dopo il sort), elimina gli altri
