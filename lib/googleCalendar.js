...
function extractUnjustifiedAttendance(assenzeData) {
    ...
    return [...assenze, ...ritardi, ...uscite].filter(item => {
        ...
        if (item.giustificata === true) return false;
        if (item.daGiustificare === false) return false;
        return true;
    });
}
...
function attendanceDescriptionLines(entries) {
    ...
    if (entries.length > 12) {
        lines.push(`• ...altri ${entries.length - 12} eventi da giustificare`);
    }
    return lines.join('\n');
}
...
async function syncUnjustifiedAttendanceReminders(assenzeData, calendarId = 'primary', auth, options = {}) {
    ...
    try {
        ...
        const existingRes = await calendar.events.list({
            calendarId,
            timeMin: `${reminderDate}T00:00:00.000Z`,
            timeMax: `${reminderDate}T23:59:59.999Z`,
            maxResults: 50,
            singleEvents: true,
        ...
        for (const hour of reminderHours) {
            ...
            const body = {
                summary,
                description,
                start: { dateTime: `${reminderDate}T${String(hour).padStart(2, '0')}:00:00`, timeZone: 'Europe/Rome' },
                end: { dateTime: `${reminderDate}T${String(hour + 1).padStart(2, '0')}:00:00`, timeZone: 'Europe/Rome' },
                colorId: '11',
                extendedProperties: {
                    private: {
                        source: 'g-connect-attendance-reminder',
