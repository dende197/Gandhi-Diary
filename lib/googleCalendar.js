...
async function syncTasksToCalendar(tasks) {
    ...
    console.log(`Filtro compiti futuri attivo. Data odierna (Roma): ${oggi.toISOString()}`);
    ...
    console.log(`Compiti totali da Argo: ${tasks.length}`);
    console.log(`Compiti futuri (da oggi): ${tasksToProcess.length}`);
    console.log(`Compiti filtrati (passati): ${results.filtered}`);

    // Safety limit to avoid Vercel timeouts for very active accounts
    const slice = tasksToProcess.slice(0, 50);
...
/**
 * Deletes all homework events prior to today.
 * Events are identified by having a summary starting with "[MATERIA]:" or bracket pattern.
 */
async function cleanOldEvents() {
    ...
    console.log(`Cleanup eventi passati iniziato. Soglia: ${oggi.toISOString()}`);

    try {
        // List events from the beginning of school year (e.g. 2024-09-01) 
        // until yesterday
        ...
        const events = res.data.items || [];
        // Filter events that look like our homework (starting with "[")
        const targets = events.filter(e => e.summary && e.summary.startsWith('['));

        console.log(`Trovati ${events.length} eventi totali passati, di cui ${targets.length} compiti da eliminare.`);

        let deleted = 0;
        for (const event of targets) {
            try {
                await calendar.events.delete({ auth, calendarId, eventId: event.id });
                deleted++;
            } catch (err) {
                console.error(`Errore eliminazione evento ${event.id}:`, err.message);
            }
        }

        return { success: true, deleted, total_past_checked: events.length };
    } catch (e) {
