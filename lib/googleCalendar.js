...
function isAttendanceEntryJustified(item) {
    ...
    const giustificataFlag = parseBooleanFlag(item.giustificata ?? item.flgGiustificata);
    const daGiustificareFlag = parseBooleanFlag(item.daGiustificare ?? item.flgDaGiustificare);
    const hasGiustificationDate = !!(
        String(item.datGiustificazione || '').trim() ||
        String(item.dataGiustificazione || '').trim()
    );

    if (daGiustificareFlag === true) return false;
    if (giustificataFlag !== null) return giustificataFlag;
    if (daGiustificareFlag === false) return true;
    if (hasGiustificationDate) return true;
    return false;
}
...
async function syncUnjustifiedAttendanceReminders(assenzeData, calendarId = 'primary', auth, options = {}) {
    ...
    if (pendingEntries.length === 0) return results;
