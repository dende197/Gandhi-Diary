...
async function syncVerificheToCalendar(verifiche, calendarId = 'primary', auth) {
    ...
    try {
        ...
        for (const v of toProcess) {
            const materia = (v.materia || 'VERIFICA').toUpperCase();
            ...
            const tipoLabel = v.tipo === 'scritta' ? ' 📝 scritta' : v.tipo === 'orale' ? ' 🗣 orale' : '';
            const summary = `[VERIFICA ${materia}]${tipoLabel}: ${text || materia}`;
