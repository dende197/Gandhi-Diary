 ...
 * Accepts tokens from the current and previous time windows (graceful overlap).
 * Default: 4 windows (72-96h, comfortably covering weekend gaps).
 * 
 * @param {object} req - HTTP request object
 * @param {string} userId - User identifier
 * @param {number} [maxWindows=4] - Number of 24h windows to check
 * @returns {boolean}
 */
function verifySessionToken(req, userId, maxWindows = 2) {
    const key = _getSessionKey();
    if (!key) return false;
    ...
    const count = Math.max(1, parseInt(maxWindows, 10) || 2);
