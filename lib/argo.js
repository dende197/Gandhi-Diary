...
function extractAssenzeFromDashboard(dashboardData) {
    const ORE_PER_GIORNO = 5; // Standard Italian high school hours per day
    const result = {
        assenze: [],
        ritardi: [],
        uscite: [],
        ...
        oreAssenzaTotali: 0,
        daGiustificare: 0,
        _debug: {}  // debug info for frontend
    };
        ...
        for (const blocco of datiList) {
            ...
            for (const a of assenzeRaw) {
                ...
                if (/RITARD|INGRESSO/i.test(tipo) || /RITARD/i.test(nota)) {
                    ...
                    // Full day absence = ORE_PER_GIORNO hours
                    result.oreAssenzaTotali += ORE_PER_GIORNO;
                }
