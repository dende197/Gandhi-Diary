...
async function getDashboard(headers) {
    try {
        const oggi = new Date();
        // Extend to 180 days to capture full semester absences
        const startRange = new Date(oggi);
        startRange.setDate(oggi.getDate() - 180);
...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        const addRawTask = (date, materia, testo) => {
            if (!date || !testo) return;
            if (!rawHomework[date]) rawHomework[date] = [];
            const isDup = rawHomework[date].some(t => t.materia === materia && t.testo === testo);
            if (!isDup) rawHomework[date].push({ materia, testo });
        };
        ...
        for (const blocco of dati) {
            ...
            for (const element of registro) {
                ...
                for (const compito of (element.compiti || [])) {
                    ...
                    addRawTask(dataConsegna, materia, compito.desCompito || compito.compito || '');
                }
...
function extractAssenzeFromDashboard(dashboardData) {
    const result = {
        assenze: [],       // full-day absences
        ritardi: [],       // late arrivals
        uscite: [],        // early exits
        note: [],          // generic notes / disciplinary
        totaleAssenze: 0,
        totaleRitardi: 0,
        totaleUscite: 0,
        oreAssenzaTotali: 0,
    ...
    try {
        ...
        if (datiList.length > 0) {
            ...
            // Also check for assenze-like keys
            for (const k of sampleKeys) {
                if (/assen|ritard|uscit|appell|ingress|presenz|event/i.test(k)) {
                    const sample = datiList[0][k];
                    debugLog(`🔍 Key "${k}" sample`, JSON.stringify(Array.isArray(sample) ? sample.slice(0, 2) : sample).substring(0, 500));
                }
            }
        }
        
        // Also check top-level dashboard keys
        const topKeys = Object.keys(dashboardData?.data || dashboardData || {});
        debugLog('🔑 Dashboard top-level keys', JSON.stringify(topKeys));

        for (const blocco of datiList) {
            const datGiorno = blocco.datGiorno || '';

            // 1. Assenze — scan ALL possible key names from DidUp
            ...
            if (assenzeRaw.length > 0) {
                debugLog('📋 Raw assenza sample', JSON.stringify(assenzeRaw[0]).substring(0, 800));
            }
            for (const a of assenzeRaw) {
                const tipo = (a.codEvento || a.tipo || a.tipoEvento || a.descrizione || '').toUpperCase();
                const giustificata = !!(a.flgDaGiustificare === false || a.giustificata || a.datGiustificazione || a.flgGiustificata);
                const daGiust = a.flgDaGiustificare === true || a.registpiuGiwornoGiustworka || (!a.datGiustificazione && !a.giustificata && !a.flgGiustificata);
                ...
                const nota = a.desAnnotazione || a.desMotivo || a.note || a.motivo || '';
                const data = a.datGiorno || a.datAssenza || a.datEvento || datGiorno;

                const entry = {
                    id: generateStableId(`${tipo}-${data}-${oraInizio}`),
                    data,
                    tipo: 'assenza',
                    giustificata,
                ...
                // Classify by type
                if (/RITARD|INGRESSO/i.test(tipo) || /RITARD/i.test(nota)) {
                    entry.tipo = 'ritardo';
                    result.ritardi.push(entry);
                    result.totaleRitardi++;
                } else if (/USCIT|ANTICIPAT/i.test(tipo) || /USCIT/i.test(nota)) {
                    entry.tipo = 'uscita';
                    ...
                    result.oreAssenzaTotali += numOre;
                }

                if (!giustificata) result.daGiustificare++;
            }

            // 2. Note disciplinari — try all possible field names
            const noteRaw = blocco.noteDisciplinari || blocco.note || blocco.noteGeneriche || [];
            for (const n of noteRaw) {
                // Try ALL possible text fields from DidUp
                const testo = n.desAnnotazione || n.desNota || n.desNotaDisciplinare || 
                ...
                // Debug: dump first note to see field names
                if (result.note.length === 0) {
                    debugLog('📝 Raw nota sample', JSON.stringify(n).substring(0, 500));
                }
        ...
        debugLog(`✅ Assenze extracted: ${result.totaleAssenze} assenze, ${result.totaleRitardi} ritardi, ${result.totaleUscite} uscite, ${result.daGiustificare} da giustificare`);
...
function extractVerificheFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            // 1. Scan promemoria + attività pianificate for verifiche
            ...
            for (const p of promemoriaItems) {
                ...
                if (VERIFICA_REGEX.test(testo)) {
                    const materia = p.desMateria || p.materia || '';
