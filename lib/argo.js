...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        const addRawTask = (dueDate, materia, testo, assignedDate) => {
            const date = dueDate || assignedDate;
            ...
            if (!isDup) rawHomework[date].push({ materia, testo, assignedDate: assignedDate || date });
        };
        ...
        for (const blocco of dati) {
            ...
            for (const element of registro) {
                ...
                for (const compito of (element.compiti || [])) {
                    ...
                    addRawTask(dataConsegna, materia, compito.desCompito || compito.compito || '', datGiorno);
                }
