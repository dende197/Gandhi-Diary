...
const {
    ...
    generateStableId, generatePid, CLASS_REGEX, DEBUG_MODE
} = require('./helpers');
...
const HOMEWORK_LIKE_ACTIVITY_PATTERN = /compit|verific|test|interrogazion[ei]?|prova|consegn/i;

async function getDashboard(headers) {
    try {
        const oggi = new Date();
        // Extend to 300 days to capture history from September onwards
        const startRange = new Date(oggi);
        startRange.setDate(oggi.getDate() - 300);
        const startDate = startRange.toISOString()
            .replace('T', ' ')
            .substring(0, 19);
        const payload = {
        ...
        // ✅ DIAGNOSTIC LOGGING — always active (remove after debugging)
        const dati = res.data?.data?.dati || res.data?.dati || [];
        console.log('📦 [DIAG] Dashboard response: dati.length =', dati.length);
        console.log('📦 [DIAG] res.data top keys:', Object.keys(res.data || {}));
        if (res.data?.data) console.log('📦 [DIAG] res.data.data top keys:', Object.keys(res.data.data));
        if (dati.length > 0) {
            const primo = dati[0];
            const keys = Object.keys(primo);
            console.log('📦 [DIAG] BLOCCO[0] KEYS:', JSON.stringify(keys));
            console.log('📦 [DIAG] BLOCCO[0] datGiorno:', primo.datGiorno);

            // Argomenti
            const arg = primo.argomenti;
            console.log('📦 [DIAG] argomenti: isArray=', Array.isArray(arg), 'length=', Array.isArray(arg) ? arg.length : 'N/A');
            if (Array.isArray(arg) && arg.length > 0) {
                console.log('📦 [DIAG] argomenti[0] KEYS:', JSON.stringify(Object.keys(arg[0])));
                console.log('📦 [DIAG] argomenti[0] SAMPLE:', JSON.stringify(arg[0]).substring(0, 600));
            }

            // Attività
            const att = primo.attivita;
            console.log('📦 [DIAG] attivita: isArray=', Array.isArray(att), 'length=', Array.isArray(att) ? att.length : 'N/A', 'value=', att === undefined ? 'undefined' : att === null ? 'null' : typeof att);
            if (Array.isArray(att) && att.length > 0) {
                console.log('📦 [DIAG] attivita[0] KEYS:', JSON.stringify(Object.keys(att[0])));
                console.log('📦 [DIAG] attivita[0] SAMPLE:', JSON.stringify(att[0]).substring(0, 600));
            }

            // Registro
            const reg = primo.registro;
            console.log('📦 [DIAG] registro: isArray=', Array.isArray(reg), 'length=', Array.isArray(reg) ? reg.length : 'N/A');
            if (Array.isArray(reg) && reg.length > 0) {
                console.log('📦 [DIAG] registro[0] KEYS:', JSON.stringify(Object.keys(reg[0])));
                console.log('📦 [DIAG] registro[0] SAMPLE:', JSON.stringify(reg[0]).substring(0, 800));
                // Check nested arrays inside registro
                const r0 = reg[0];
                if (r0.attivita) console.log('📦 [DIAG] registro[0].attivita:', JSON.stringify(r0.attivita).substring(0, 400));
                if (r0.argomenti) console.log('📦 [DIAG] registro[0].argomenti:', JSON.stringify(r0.argomenti).substring(0, 400));
                if (r0.compiti) console.log('📦 [DIAG] registro[0].compiti:', JSON.stringify(r0.compiti).substring(0, 400));
            }

            // Dump all keys that contain 'attiv' or 'argom' or 'lez' (case-insensitive)
            const activityKeys = keys.filter(k => /attiv|argom|lez|svolt|regist/i.test(k));
            if (activityKeys.length > 0) {
                console.log('📦 [DIAG] Activity-related keys in blocco:', JSON.stringify(activityKeys));
                activityKeys.forEach(k => {
                    const val = primo[k];
                    const preview = Array.isArray(val) ? `Array(${val.length})` : typeof val;
                    console.log(`📦 [DIAG]   ${k}: ${preview}`, Array.isArray(val) && val.length > 0 ? JSON.stringify(val[0]).substring(0, 300) : '');
                });
            }

            // Also check a middle blocco for variety
            if (dati.length > 5) {
                const mid = dati[Math.floor(dati.length / 2)];
                const midReg = mid.registro;
                const midArg = mid.argomenti;
                console.log(`📦 [DIAG] MID BLOCCO (idx=${Math.floor(dati.length / 2)}): argomenti=${Array.isArray(midArg) ? midArg.length : 'N/A'}, registro=${Array.isArray(midReg) ? midReg.length : 'N/A'}`);
                if (Array.isArray(midReg) && midReg.length > 0) {
                    console.log('📦 [DIAG] MID registro[0] SAMPLE:', JSON.stringify(midReg[0]).substring(0, 500));
                }
            }
        } else {
            console.log('📦 [DIAG] ⚠️ dati array is EMPTY! Full response keys:', JSON.stringify(Object.keys(res.data || {})));
            console.log('📦 [DIAG] Full response sample:', JSON.stringify(res.data).substring(0, 1500));
        }
        // ✅ END DIAGNOSTIC
...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        const addRawTask = (dueDate, materia, testo, assignedDate, assignedAt) => {
            const groupingDate = dueDate || assignedDate;
            ...
            if (!isDup) rawHomework[groupingDate].push({ materia, testo, assignedDate: assignedDate || groupingDate, assignedAt: assignedAt || null });
        };
        ...
        for (const blocco of dati) {
            ...
            for (const element of registro) {
                ...
                for (const compito of (element.compiti || [])) {
                    ...
                    addRawTask(dataConsegna, materia, compito.desCompito || compito.compito || '', datGiorno, assignedAt);
                }
                ...
                for (const note of notesArray) {
                    ...
                    if (testo && textDate) addRawTask(textDate, materia, testo);
                }
            }
            ...
            for (const p of notices) {
                ...
                const textDate = parseDateFromText(testo);
                if (testo && textDate) {
                    addRawTask(textDate, p.materia || p.desMateria || 'AVVISO', testo);
                }
            }
            ...
            for (const a of (blocco.argomenti || [])) {
                ...
                if (testo && isTask) {
                    const textDate = parseDateFromText(testo);
                    addRawTask(textDate || datGiorno, a.materia || a.desMateria || 'ARGOMENTO', testo);
                }
            }
        }
...
function extractClassActivitiesFromDashboard(dashboardData) {
    const activities = [];
    ...
    const pushActivity = (rowDate, subject, content, source = 'generic', useLegacyId = false) => {
        const normalizedContent = normalizeText(content);
        if (!normalizedContent) return;
        const normalizedSubject = normalizeText(subject) || 'Materia';
        const normalizedDate = normalizeText(rowDate);
    ...
    try {
        ...
        let _diagCounts = { blocchi: datiList.length, argomenti: 0, attivita: 0, registro: 0, registroNested: 0, pushed: 0 };
        ...
        for (const blocco of datiList) {
            ...
            fromRegistro.forEach((entry) => {
                ...
                // Direct fields on registro entry
                ...
                if (argLezione) _diagCounts.pushed++;
                if (attSvolta) _diagCounts.pushed++;
                pushActivity(rowDate, subject, argLezione, 'registro-argomento');
                pushActivity(rowDate, subject, attSvolta, 'registro-attivita');

                // Also try: contenuto, descrizione, testo (broader fallback)
                const fallbackContent = entry?.contenuto || entry?.descrizione || entry?.testo || '';
                if (fallbackContent && fallbackContent !== argLezione && fallbackContent !== attSvolta) {
                ...
                nestedArgomenti.forEach((a) => {
                    pushActivity(rowDate, a?.desMateria || subject, a?.desArgomento || a?.argomento || '', 'registro-argomento-nested');
                });

                const nestedAttivita = Array.isArray(entry?.attivita) ? entry.attivita : [];
                _diagCounts.registroNested += nestedAttivita.length;
                nestedAttivita.forEach((a) => {
                    const content = a?.desAttivita || a?.desDescrizione || a?.desAttivitaSvolta || a?.testo || '';
                    const subj = a?.desMateria || subject;
                ...
                notesArray.forEach((note) => {
                    ...
                    // Homework-like notes are excluded from class activities because homework extraction handles them separately.
        ...
        console.log('📊 [DIAG] extractClassActivities counts:', JSON.stringify(_diagCounts));
        console.log('📊 [DIAG] raw activities before dedup:', activities.length);
    } catch (e) {
        console.error('⚠️ Errore estrazione attività svolte', e.message, e.stack);
    }
    const seen = new Set();
    ...
    console.log('📊 [DIAG] extractClassActivities FINAL result:', result.length, 'activities');
    if (result.length > 0) {
        console.log('📊 [DIAG] Sample activity:', JSON.stringify(result[0]));
    }
    return result;
}
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            for (const a of assenzeRaw) {
                ...
                const giustificata = !!(a.giustificata === 'S' || a.giustificata === true || 
                                        a.datGiustificazione || a.dataGiustificazione || a.flgGiustificata);
