...
function parseDateFromText(text, referenceDateOrYear = new Date().getFullYear()) {
    ...
    // 3. Month names in Italian (e.g. "15 marzo", "per il 22 settembre", "entro il 2 maggio")
    const monthNames = [
    ...
    for (let i = 0; i < monthNames.length; i++) {
        ...
        if (lower.includes(mName)) {
            const mRegex = new RegExp(`(?:per\\s+(?:il\\s+)?|entro\\s+(?:il\\s+)?|consegna\\s+(?:il\\s+)?|scadenza\\s+(?:il\\s+)?|il\\s+|del\\s+|giorno\\s+)?\\b(\\d{1,2})\\s+${mName}`, 'i');
    ...
    // 4. Explicit date pattern: "per il 15/03", "per 22/09", "entro 20/04", "del 10/05", "consegna 12/03", "per lunedì 22/09"
...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        const addRawTask = (dueDate, materia, testo, assignedDate, assignedAt) => {
            ...
            const cleanMateria = String(materia || 'Generico').trim() || 'Generico';
            if (!rawHomework[groupingDate]) rawHomework[groupingDate] = [];
            const isDup = rawHomework[groupingDate].some(t => t.materia === cleanMateria && t.testo === cleanText);
            if (!isDup) {
        ...
        const processCompitoItem = (compito, defaultMateria, fallbackDate) => {
            ...
            if (typeof compito === 'object') {
                ...
                const explicitDueDate = compito.dataConsegna || compito.datConsegna ||
                    ...
                    compito.datScadenza || compito.dataFine || compito.datFine || compito.data || compito.datGiorno;
        ...
        for (const blocco of dati) {
            ...
            for (const act of plannedActivities) {
                ...
                const isTaskLike = HOMEWORK_LIKE_ACTIVITY_PATTERN.test(actText) || /compit|studiare|esercizi|verific|consegn|leggere|tav\.\s*\d+/i.test(actText);
            ...
            for (const element of registro) {
                const materia = element.materia || element.desMateria || 'Generico';
                ...
                const elemAttivita = typeof element.attivita === 'string' ? element.attivita.trim() : '';
                ...
                for (const note of notesArray) {
                    ...
                    const isTaskLike = HOMEWORK_LIKE_ACTIVITY_PATTERN.test(testo) || /compit|verific|test|interrogazion|consegn/i.test(testo);
            ...
            // E. Argomenti with task/homework keywords
            for (const a of (blocco.argomenti || [])) {
                const testo = (a.desArgomento || a.argomento || '').trim();
                const isTask = /compit|verific|test|interrogazion|prova|consegn|studiare/i.test(testo);
                if (testo && isTask) {
                    const textDate = parseDateFromText(testo, datGiorno);
