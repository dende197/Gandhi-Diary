...
function extractAssenzeFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            for (const a of assenzeRaw) {
                ...
                const entry = {
                    ...
                    oraInizio, oraFine, numOre, nota
                };

                if (/RITARD|INGRESSO|^R$/i.test(tipo) || /RITARD/i.test(nota)) {
                    entry.tipo = 'ritardo';
                    result.ritardi.push(entry);
                    result.totaleRitardi++;
                } else if (/USCIT|ANTICIPAT|^U$/i.test(tipo) || /USCIT/i.test(nota)) {
                    entry.tipo = 'uscita';
        ...
        // Map of YYYY-MM-DD → effective school hours for that day
        ...
        for (const note of result.note) {
            ...
            // Pattern: "esce alle (ore) HH" / "uscita alle HH" / "lezioni terminano alle HH"
            ...
            if (exitMatch) {
                ...
                if (exitHour > ORA_INIZIO_SCUOLA && exitHour <= ORA_FINE_SCUOLA) {
                    ...
                    debugLog(`📐 Day modifier: ${targetDate} → ${effectiveHours}h (exit at ${exitHour}:00) from note: "${note.testo.substring(0, 60)}"`);
                }
            }

            // Pattern: "entra alle (ore) HH" / "ingresso alle HH"
            ...
            if (entryMatch) {
                ...
                if (entryHour >= ORA_INIZIO_SCUOLA && entryHour < ORA_FINE_SCUOLA) {
                    ...
                    const current = dayModifiers.get(targetDate);
                    dayModifiers.set(targetDate, current ? Math.min(current, effectiveHours) : effectiveHours);
                    debugLog(`📐 Day modifier: ${targetDate} → ${effectiveHours}h (entry at ${entryHour}:00)`);
                }
            }

            // Pattern: "assemblea d'istituto" / "assemblea di istituto"
            if (/assemblea\s+d['i]?\s*istituto/i.test(testo)) {
                dayModifiers.set(targetDate, 1);
                debugLog(`📐 Day modifier: ${targetDate} → 1h (assemblea d'istituto)`);
            }

            // Pattern: "solo N ore" / "N ore di lezione"
            const oreMatch = testo.match(/(?:solo|soltanto)\s+(\d)\s+or[ea]/);
            if (oreMatch) {
                const hours = parseInt(oreMatch[1]);
                dayModifiers.set(targetDate, hours);
                debugLog(`📐 Day modifier: ${targetDate} → ${hours}h (explicit)`);
            }
        }

        result._debug.dayModifiers = Object.fromEntries(dayModifiers);
        ...
        for (const assenza of result.assenze) {
            ...
            const effectiveHours = dayModifiers.get(dateKey) || ORE_PER_GIORNO;
            assenza.oreEffettive = effectiveHours;
            result.oreAssenzaTotali += effectiveHours;
        }

        // Also count partial hours for ritardi (late = ~1h) and uscite (early exit = ~1h)
        result.oreAssenzaTotali += result.totaleRitardi * 1;
        result.oreAssenzaTotali += result.totaleUscite * 1;

        debugLog(`✅ Assenze: ${result.totaleAssenze} assenze (${result.oreAssenzaTotali}h), ${result.totaleRitardi} ritardi, ${result.totaleUscite} uscite, ${dayModifiers.size} day modifiers applied`);
