...
async function getDashboard(headers) {
    try {
        ...
        if (dati.length > 0) {
            const primo = dati[0];
            debugLog('📦 BLOCCO KEYS:', Object.keys(primo));
            if (Array.isArray(primo.registro) && primo.registro.length > 0) {
                const reg = primo.registro[0];
                debugLog('📦 REGISTRO[0] KEYS:', Object.keys(reg));
                debugLog('📦 REGISTRO[0] SAMPLE:', JSON.stringify(reg, null, 2).substring(0, 1000));
            } else {
                debugLog('📦 blocco.registro:', primo.registro);
            }
        }
        // ✅ FINE DEBUG
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            fromRegistro.forEach((entry) => {
                ...
                pushActivity(rowDate, subject, entry?.desArgomentoLezione || entry?.desArgomento || entry?.argomento || '', 'registro-argomento');
                pushActivity(rowDate, subject, entry?.desAttivitaSvolta || entry?.desAttivita || '', 'registro-attivita');
        ...
        debugLog('⚠️ Errore estrazione attività svolte', e.message);
    }
    const seen = new Set();
