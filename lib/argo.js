        ...
        for (const blocco of dati) {
            ...
            for (const p of notices) {
                ...
                if (testo) {
                    ...
                    // Crucial Fix: For annotations, only add if a specific date is found in text
                    // otherwise it's just a general note tied to the teacher's login day
                    if (textDate) {
                        addRawTask(textDate, materia, testo);
                    }
                }
            }
