...
function extractClassActivitiesFromDashboard(dashboardData) {
    const activities = [];
    const isHomeworkLikeText = (text) => /compit|verific|test|interrogazion[ei]?|prova|consegn/i.test(String(text || ''));
    ...
    const pushActivity = (rowDate, subject, content) => {
        ...
        activities.push({
            id: generateStableId(`act-${normalizedDate}-${normalizedSubject}-${normalizedContent}`),
            date: normalizedDate,
        ...
        for (const blocco of datiList) {
            ...
            fromRegistro.forEach((entry) => {
                ...
                notesArray.forEach((note) => {
                    ...
                    if (!noteText || isHomeworkLikeText(noteText)) return;
