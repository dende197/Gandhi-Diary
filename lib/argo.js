    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            for (const a of assenzeRaw) {
                ...
                const giustificataFlag = parseBooleanFlag(a.giustificata ?? a.flgGiustificata);
                const daGiustificareFlag = parseBooleanFlag(a.daGiustificare ?? a.flgDaGiustificare);
                const hasGiustificationDate = !!(
                    String(a.datGiustificazione || '').trim() ||
                    String(a.dataGiustificazione || '').trim()
                );
                const giustificata = daGiustificareFlag === true
                    ? false
                    : (giustificataFlag !== null
                        ? giustificataFlag
                        : (daGiustificareFlag === false || hasGiustificationDate));
