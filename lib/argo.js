...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of dati) {
            ...
            // 2. Promemoria (molto usati per Verifiche/Test)
            const promemoria = blocco.promemoria || [];
            for (const p of promemoria) {
                const testo = p.desAnnotazioni || p.testo || p.desMessaggio || '';
                if (testo) {
                    addRawTask(datGiorno, 'PROMEMORIA', testo);
                }
            }
            ...
            for (const a of argomenti) {
                ...
                if (testo && (testo.toLowerCase().includes('compit') || testo.toLowerCase().includes('verific'))) {
                    addRawTask(datGiorno, 'ARGOMENTO', testo);
                }
            }
        }
