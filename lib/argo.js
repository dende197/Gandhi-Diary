...
async function getDashboard(headers) {
    try {
        ...
        const startDate = trentaGiorniFa.toISOString()
            .replace('T', ' ')
            .substring(0, 19);
...
function extractAssenzeFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            // 1. Assenze array
            const assenzeRaw = blocco.assenze || [];
            
            // Debug: log first raw assenza entry to understand field structure
            if (assenzeRaw.length > 0) {
                debugLog('📋 Raw assenza sample', JSON.stringify(assenzeRaw[0]).substring(0, 500));
            }
            for (const a of assenzeRaw) {
                const tipo = (a.codEvento || a.tipo || a.tipoEvento || '').toUpperCase();
                const giustificata = !!(a.flgDaGiustificare === false || a.giustificata || a.datGiustificazione);
                const daGiust = a.flgDaGiustificare === true || (!a.datGiustificazione && !a.giustificata);
                const oraInizio = a.oraInizio || a.numOra || '';
                const oraFine = a.oraFine || '';
                const numOre = a.numOre || a.durataOre || 1;
                const nota = a.desAnnotazione || a.note || a.motivo || '';
                const data = a.datGiorno || a.datAssenza || datGiorno;
                ...
                if (daGiust) result.daGiustificare++;
            }

            // 2. Note disciplinari
            const noteRaw = blocco.noteDisciplinari || blocco.note || [];
            for (const n of noteRaw) {
                const testo = n.desNota || n.testo || n.desAnnotazione || '';
                const autore = n.docente || n.desDocente || n.desMittente || '';
                const data = n.datNota || n.datGiorno || datGiorno;

                // Check if notes contain absence corrections
                // e.g. "Uscita autorizzata alle ore 12:00" or "Ingresso alla seconda ora"
                ...
                result.note.push({
                    id: generateStableId(`nota-${testo}-${data}`),
                    data,
                    testo,
                    autore,
                    isAbsenceCorrection,
...
function extractVerificheFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            // 1. Scan promemoria for verifiche
            const promemoriaItems = [...(blocco.promemoria || []), ...(blocco.bachecaAlunno || [])];
