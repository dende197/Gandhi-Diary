...
function globalSearch(obj, target, results = []) {
    if (!obj) return results;
    const str = JSON.stringify(obj).toLowerCase();
    if (str.includes(target.toLowerCase())) {
        if (Array.isArray(obj)) {
            obj.forEach(item => globalSearch(item, target, results));
        } else if (typeof obj === 'object') {
            // If it's a small object (like a task/note) and contains the target, save it
            if (str.length < 2000) {
                results.push(obj);
            } else {
                Object.values(obj).forEach(val => globalSearch(val, target, results));
            }
        }
    }
    return results;
...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        // Debug hunt
        const found14 = globalSearch(dashboardData, '14/03');
        const foundVerifiche = globalSearch(dashboardData, 'verifica');
        if (found14.length > 0 || foundVerifiche.length > 0) {
            console.log('--- GLOBAL SEARCH HITS ---');
            console.log('14/03 Hits:', found14.length);
            console.log('Verifica Hits:', foundVerifiche.length);
        }


        // Helper to add a task with basic deduplication
        const addRawTask = (date, materia, testo) => {
            if (!date || !testo) return;
            if (!rawHomework[date]) rawHomework[date] = [];

            // Avoid exact duplicates for the same date/materia
            const isDup = rawHomework[date].some(t => t.materia === materia && t.testo === testo);
            if (!isDup) {
                rawHomework[date].push({ materia, testo });
            }
        };

        const dati = dashboardData?.data?.dati || dashboardData?.dati || [];
        for (const blocco of dati) {
            const datGiorno = blocco.datGiorno; // Formato YYYY-MM-DD solitamente

            const rawStr = JSON.stringify(blocco);
            if (rawStr.toLowerCase().includes('verifica') || datGiorno === '2026-03-14') {
                console.log(`DEBUG TARGET [${datGiorno}]:`, rawStr.substring(0, 3000));
            }

            // 1. Compiti e Annotazioni dal registro del giorno
            ...
            for (const element of registro) {
                ...
                // Annotazioni giornaliere (spesso usate per verifiche)
                ...
                for (const note of notesArray) {
                    if (typeof note === 'string' && note) addRawTask(datGiorno, materia, note);
                    else if (note && note.desAnnotazione) addRawTask(datGiorno, materia, note.desAnnotazione);
                }
            }

            // 2. Promemoria e Bacheca (Annotazioni e Avvisi generali)
            ...
            for (const p of notices) {
                ...
                if (testo) {
                    const materia = p.materia || p.desMateria || 'AVVISO';
                    addRawTask(datGiorno, materia, testo);
                }
            }

            // 3. Argomenti (a volte i compiti sono scritti qui)
            const argomenti = blocco.argomenti || [];
            for (const a of argomenti) {
                const testo = a.desArgomento || a.argomento || '';
                // Pick up only if it looks like a task or test to avoid filling the calendar with every lesson topic
                const isTask = /compit|verific|test|interrogazion|prova|consegn/i.test(testo);
                if (testo && isTask) {
                    const materia = a.materia || a.desMateria || 'ARGOMENTO';
                    addRawTask(datGiorno, materia, testo);
                }
            }
        }

        // Convert grouped data to final task objects
        for (const [dateStr, items] of Object.entries(rawHomework)) {
            items.forEach((item) => {
                const stableId = generateStableId(`${item.testo}-${item.materia}-${dateStr}`);
                tasksData.push({
                    id: stableId,
                    text: item.testo,
        ...
        debugLog('⚠️ Errore estrazione compiti estesa', e.message);
    }
    return tasksData;
}
