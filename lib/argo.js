...
async function getDashboard(headers) {
    try {
        ...
        const payload = {
            ...
            opzioni: JSON.stringify(dashboardOptions)
        };
...
function extractClassActivitiesFromDashboard(dashboardData) {
    const activities = [];
    const shouldFilterAsHomework = (text) => /compit|verific|test|interrogazion[ei]?|prova|consegn/i.test(String(text || ''));
    const normalizeText = (text) => String(text || '').trim();
    const normalizeKeyPart = (text) => normalizeText(text).toLowerCase().replace(/\s+/g, ' ');
    const pushActivity = (rowDate, subject, content) => {
        ...
        activities.push({
            id: generateStableId(`act-${normalizedDate}-${dedupeSubject}-${dedupeContent}`),
            date: normalizedDate,
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            fromArgomenti.forEach((entry) => {
                ...
                pushActivity(rowDate, subject, content);
            });
            ...
            fromRegistro.forEach((entry) => {
                ...
                pushActivity(rowDate, subject, entry?.desArgomentoLezione || entry?.desArgomento || entry?.argomento || '');
                pushActivity(rowDate, subject, entry?.desAttivitaSvolta || entry?.desAttivita || '');
                ...
                notesArray.forEach((note) => {
                    ...
                    // Homework-like notes are excluded here because homework extraction already handles them.
                    if (!noteText || shouldFilterAsHomework(noteText)) return;
                    pushActivity(rowDate, subject, noteText);
                });
            });
        }
    } catch (e) {
