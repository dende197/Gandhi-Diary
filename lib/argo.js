...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        const addRawTask = (dueDate, materia, testo, assignedDate, assignedAt) => {
            ...
            if (!isDup) rawHomework[groupingDate].push({ materia, testo, assignedDate: normalizedAssignedDate || null, assignedAt: assignedAt || null });
        };
        ...
        for (const [dateStr, items] of Object.entries(rawHomework)) {
            items.forEach((item) => {
                tasksData.push({
                    ...
                    due_date: dateStr,
                    assigned_date: item.assignedDate || null,
