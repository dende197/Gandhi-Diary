...
function parseDateFromText(text, year = new Date().getFullYear()) {
    ...
    // 1. Month names (e.g. "15 marzo", "2 aprile", "25 maggio")
    const monthNames = ['gennaio', 'febbraio', 'marzo', 'aprile', 'maggio', 'giugno', 'luglio', 'agosto', 'settembre', 'ottobre', 'novembre', 'dicembre'];
    for (let i = 0; i < monthNames.length; i++) {
        if (text.toLowerCase().includes(monthNames[i])) {
            const dayMatch = text.match(new RegExp(`(?:per\\s+il|entro\\s+il|il|del|giorno)?\\s*(\\d{1,2})\\s+${monthNames[i]}`, 'i'));
            if (dayMatch) {
    ...
    // 2. Explicit date pattern: "per il 15/03", "entro il 20/04", "del 10/05", "giorno 12/03"
    // Requires date context (il, del, entro, per, data, giorno) to avoid matching exercise numbers like "es. 2/4" or "pag. 3/5"
    const dateMatch = text.match(/(?:per\s+il|entro\s+il|il|del|data|giorno)\s*(\d{1,2})[\/\.](0?[1-9]|1[0-2])\b/i);
    if (dateMatch) {
...
function extractHomeworkFromDashboard(dashboardData) {
    ...
    try {
        ...
        for (const blocco of dati) {
            ...
            // 1. Compiti direttamente nel blocco del giorno
            const directCompiti = [
            ...
            for (const compito of directCompiti) {
                if (!compito || typeof compito !== 'object') continue;
                const materia = compito.materia || compito.desMateria || 'Generico';
                ...
                const testo = compito.desCompito || compito.compito || compito.testo || compito.desOggetto || '';
                addRawTask(dataConsegna, materia, testo, assignedDate, assignedAt);
            }

            // 2. Compiti dal registro
            ...
            for (const element of registro) {
                ...
                const elementCompiti = [
                    ...
                    ...(element.compito ? (Array.isArray(element.compito) ? element.compito : [element.compito]) : [])
                ];
                for (const compito of elementCompiti) {
                    ...
                    const assignedAt = compito.datOraIns || compito.datOraInserimento || compito.dataOraInserimento || compito.tsInserimento || null;
                    const assignedDate = resolveAssignedDate(compito, datGiorno);
                    const testo = compito.desCompito || compito.compito || compito.testo || '';
                    addRawTask(dataConsegna, materia, testo, assignedDate, assignedAt);
                }
            ...
            // 3. Promemoria e Bacheca: solo se contengono una data esplicita
            ...
            // 4. Argomenti con keyword compito/verifica
