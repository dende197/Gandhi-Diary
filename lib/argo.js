...
function extractClassActivitiesFromDashboard(dashboardData) {
    const activities = [];
    const shouldFilterAsHomework = (text) => HOMEWORK_LIKE_ACTIVITY_PATTERN.test(String(text || ''));
    const normalizeText = (text) => String(text || '').trim();
    const normalizeKeyPart = (text) => normalizeText(text).toLowerCase().replace(/\s+/g, ' ');
    const pushActivity = (rowDate, subject, content, source = 'generic') => {
        ...
        activities.push({
            id: generateStableId(`act-${source}-${normalizedDate}-${dedupeSubject}-${dedupeContent}`),
            date: normalizedDate,
    ...
    try {
        ...
        for (const blocco of datiList) {
            ...
            fromArgomenti.forEach((entry) => {
                ...
                const content = entry?.desArgomento || entry?.argomento || '';
                pushActivity(rowDate, subject, content, 'argomenti');
            });
            ...
            fromAttivita.forEach((entry) => {
                const content = entry?.desAttivita || entry?.desDescrizione || entry?.desAnnotazioni || entry?.testo || '';
                const subject = entry?.desMateria || entry?.materia || 'Materia';
                const rowDate = entry?.datGiorno || date;
                pushActivity(rowDate, subject, content, 'attivita');
            });
            ...
            fromRegistro.forEach((entry) => {
                ...
                notesArray.forEach((note) => {
                    ...
                    // Homework-like notes are excluded here because extractHomeworkFromDashboard() already captures them.
                    if (!noteText || shouldFilterAsHomework(noteText)) return;
