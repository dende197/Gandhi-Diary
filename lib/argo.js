...
async function getDashboard(headers, options = {}) {
    ...
    try {
        ...
        console.log('📦 [DIAG] res.data top keys:', Object.keys(firstPayload || {}));
        if (firstPayload?.data) console.log('📦 [DIAG] res.data.data top keys:', Object.keys(firstPayload.data));
        if (dati.length > 0) {
            const primo = dati[0];
            ...
            console.log('📦 [DIAG] ⚠️ dati array is EMPTY! Full response keys:', JSON.stringify(Object.keys(firstPayload || {})));
            console.log('📦 [DIAG] Full response sample:', JSON.stringify(firstPayload).substring(0, 1500));
        }
        // ✅ END DIAGNOSTIC
...
function extractClassActivitiesFromDashboard(dashboardData, profileContext = {}) {
    ...
    const COMPLETED_ACTIVITY_PATTERN = /spiegazion|correzion|ripasso|lezione|introduzion|approfond|discuss|svolt[oaie]|interrogazion.*svolt|verifica\s+svolt/i;
    const PLANNED_ACTIVITY_PATTERN = /compit|da\s+fare|per\s+casa|consegn|scadenz|studiare|leggere|eserciz|tav\.\s*\d+/i;
    const classifyActivityType = (rowDate, content, source, forcedType) => {
        ...
        if (sourceText.includes('pianificat')) return 'pianificata';
        if (normalizedDate && normalizedDate > todayIso) return 'pianificata';
    ...
    const pushActivity = (rowDate, subject, content, source = 'generic', useLegacyId = false, forcedType = '') => {
        ...
        const id = useLegacyId
            ...
            : generateStableId(`act-${type}-${source}-${normalizedDate}-${dedupeSubject}-${dedupeContent}`);
        const item = {
            id,
            date: normalizedDate,
    ...
    try {
        ...
        console.log('📊 [DIAG] raw activities before dedup:', JSON.stringify({
            svolte: completedActivities.length,
            pianificate: plannedActivities.length
        }));
    } catch (e) {
    ...
    console.log('📊 [DIAG] extractClassActivities FINAL result:', JSON.stringify({
        svolte: resultSvolte.length,
        pianificate: resultPianificate.length
    }));
    if (resultSvolte.length > 0) {
        console.log('📊 [DIAG] FIRST attività svolta (oldest):', JSON.stringify(resultSvolte[0]));
        console.log('📊 [DIAG] LAST attività svolta (newest):', JSON.stringify(resultSvolte[resultSvolte.length - 1]));
    }
    if (resultPianificate.length > 0) {
        console.log('📊 [DIAG] LAST attività pianificata (newest):', JSON.stringify(resultPianificate[resultPianificate.length - 1]));
    }
    return {
        svolte: resultSvolte,
