    ...
    function goToSlide(index) {
        ...
        for (let i = 0; i < totalSlides; i++) {
            ...
            if (i === currentSlide) {
                ...
                dot.style.boxShadow = '0 0 10px rgba(182,196,255,0.7)';
            } else {
                dot.style.width = '5px';
...
function renderHome() {
    ...
    if (months.length >= 2) {
        ...
        performanceBarsHtml = months.slice(-6).map(m => {
            ...
            const normalizedHeight = Math.max(18, Math.min(100, ((m.avg - minAvg) / (maxAvg - minAvg)) * 100));
            const barBg = isCur 
            ...
            const barShadow = isCur ? 'box-shadow:0 0 12px rgba(182,196,255,0.6);' : '';
            return `
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;height:100%;justify-content:flex-end;">
                <span style="font-size:9px;font-weight:800;color:${isCur ? '#b6c4ff' : (m.avg >= 6 ? '#dae2fd' : '#ffb4ab')};letter-spacing:-0.02em;">${m.avg.toFixed(1)}</span>
                <div style="width:100%;max-width:28px;height:${normalizedHeight}%;background:${barBg};border-radius:6px 6px 2px 2px;border:0.5px solid ${barBorder};${barShadow}transition:height 0.4s ease;"></div>
                <span style="font-size:8px;font-weight:700;color:${isCur ? '#dae2fd' : '#8e909f'};text-transform:uppercase;letter-spacing:0.04em;">${m.label}</span>
            </div>`;
        }).join('');
        ...
        performanceBarsHtml = recent.map((v, i) => {
            ...
            const h = Math.max(18, Math.min(100, (v.val / 10) * 100));
            ...
            const barShadow = isLast ? 'box-shadow:0 0 10px rgba(182,196,255,0.55);' : '';
            return `
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;height:100%;justify-content:flex-end;">
                <span style="font-size:9px;font-weight:800;color:${isLast ? '#b6c4ff' : (isPass ? '#6ee7b7' : '#ffb4ab')};">${v.val.toFixed(1).replace(/\.0$/, '')}</span>
                <div style="width:100%;max-width:24px;height:${h}%;background:${barBg};border-radius:6px 6px 2px 2px;border:0.5px solid ${barBorder};${barShadow}"></div>
                <span style="font-size:8px;font-weight:700;color:#c4c5d6;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:30px;">${getSubjectAbbrev(v.materia)}</span>
            </div>`;
        }).join('');
    } else {
        performanceBarsHtml = `
        <div style="flex:1;display:flex;align-items:center;justify-content:center;color:#8e909f;font-size:11px;font-style:italic;">
                    ...
                    <div id="home-media-track" style="display:flex;width:100%;transition:transform 0.4s cubic-bezier(0.16,1,0.3,1);will-change:transform;">
                        ...
                        <div style="width:100%;min-width:100%;max-width:100%;flex-shrink:0;box-sizing:border-box;overflow:hidden;position:relative;padding:15px 18px 8px 18px;height:178px;display:flex;flex-direction:column;justify-content:space-between;">
                            <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:0.5px solid rgba(182,196,255,0.12);padding-bottom:5px;">
                                ...
                                <span style="font-size:10px;font-weight:700;color:#8e909f;background:rgba(23,31,51,0.8);border:0.5px solid rgba(182,196,255,0.15);padding:2px 8px;border-radius:999px;">Anno Scolastico</span>
                            </div>
                            ...
                            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;margin:auto 0;text-align:center;">
                                <span class="material-symbols-outlined" style="font-size:26px;color:rgba(182,196,255,0.3);margin-bottom:4px;">analytics</span>
                                <p style="font-size:12px;font-weight:600;color:#8e909f;margin:0;">Nessun voto registrato per l'anno scolastico</p>
                            </div>
                            `}
                        </div>
                        ...
                        <div style="width:100%;min-width:100%;max-width:100%;flex-shrink:0;box-sizing:border-box;overflow:hidden;position:relative;padding:15px 18px 8px 18px;height:178px;display:flex;flex-direction:column;justify-content:space-between;">
                            <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:0.5px solid rgba(182,196,255,0.12);padding-bottom:5px;">
                                ...
                                <span style="font-size:10px;font-weight:700;color:#ffd49c;background:rgba(255,212,156,0.12);border:1px solid rgba(255,212,156,0.3);padding:1px 7px;border-radius:999px;">
                                    ${_obiettivoProgress}% Target
                                </span>
                            </div>
